...
4. **Environment variables**: `OPENAI_API_KEY` and `ELEVENLABS_API_KEY` are **required**. Optional: `ELEVENLABS_VOICE_ID`, `ELEVENLABS_TTS_MODEL_ID` (defaults to `eleven_turbo_v2_5`), `ELEVENLABS_TTS_OUTPUT_FORMAT` (defaults to `mp3_44100_64`), `ELEVENLABS_TTS_OPTIMIZE_STREAMING_LATENCY` (defaults to `3`), `ELEVENLABS_TTS_STABILITY` (defaults to `0.4`), and `ELEVENLABS_TTS_SIMILARITY` (defaults to `1`). Missing keys cause silent failures (validation returns `matched: false`).
