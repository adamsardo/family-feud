...
Store `OPENAI_API_KEY`, `ELEVENLABS_API_KEY`, and optional `ELEVENLABS_VOICE_ID` in `.env.local`; never commit secrets. You can also set `ELEVENLABS_TTS_MODEL_ID` (defaults to `eleven_v3`) and `ELEVENLABS_TTS_OUTPUT_FORMAT` (defaults to `mp3_22050_32`) to tune quality/size. Edge routes should avoid Node-only globals and log only sanitized strings. Rotate keys immediately if exposed and document new secrets in the PR checklist.
