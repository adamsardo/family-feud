...
Store `OPENAI_API_KEY`, `ELEVENLABS_API_KEY`, and optional `ELEVENLABS_VOICE_ID` in `.env.local`; never commit secrets. You can also set `ELEVENLABS_TTS_MODEL_ID` (defaults to `eleven_turbo_v2_5`), `ELEVENLABS_TTS_OUTPUT_FORMAT` (defaults to `mp3_44100_64`), `ELEVENLABS_TTS_OPTIMIZE_STREAMING_LATENCY` (defaults to `3`), `ELEVENLABS_TTS_STABILITY` (defaults to `0.4`), and `ELEVENLABS_TTS_SIMILARITY` (defaults to `1`) to tune latency/quality trade-offs. Edge routes should avoid Node-only globals and log only sanitized strings. Rotate keys immediately if exposed and document new secrets in the PR checklist.
